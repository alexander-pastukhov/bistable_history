...
$$
h_i(t) = A e^{-t/\tau} + \frac{1}{\tau} \int_{0}^{t} e^{-(t'-t)/\tau} S(t') dt'
$$

where $A$ is chosen to satisfy the initial condition. Assuming a constant signal $S$, we obtain
$$
h_i(t) = A e^{-t/\tau} + S_i \cdot (1 - e^{-t/\tau})
$$

In our case, we are interested in a value of cumulative history following a dominance phase that starts at time $t$, ends at time $t + \Delta t$, and has a constant signal strength $S_i$. Solving for initial condition $h_i(t)$ and $S_i$:
$$h_i(t + \Delta t) = 
